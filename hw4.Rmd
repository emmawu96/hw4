...
whoex3 <- select(who3, country, iso2, iso3) %>%
           distinct() %>%
           group_by(country)
whoex3 %>% count(vars="country") %>% filter(freq>1)
...
#   summarise("cases" = sum(who5$cases)) %>%
#   ggplot(aes(x = who5$year, y = who5$cases, color = who5$country)) +
#   geom_line() +
#   facet_wrap(~who5$sex)+ theme(legend.position = "none")

group_by(who5,country, year, sex) %>%
